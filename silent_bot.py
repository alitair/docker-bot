...
class SilentBot(EventHandler):
    ...
    def delay(self, value):
        self._delay            = value
        self._frame_size       = int(self._sample_rate * SilentBot.BUFFER ) # * Bot.BYTES_PER_SAMPLE)
        self._blank_frame      = bytes([0] * self._frame_size)
        self._ideal_queue_size = max( int( (self._delay-.1)/SilentBot.BUFFER), 1 )
    ...
    def __init__(self):
        ...
        self._mic_device     = Daily.create_microphone_device("my-mic", sample_rate=self._sample_rate , channels=SilentBot.CHANNELS)
        self._speaker_device = Daily.create_speaker_device("my-speaker", sample_rate=self._sample_rate , channels=SilentBot.CHANNELS)
        ...
        while not self._app_quit:
            try:
                if self._delay <= 0 :
                    ...
                    buffer = self._buffer_queue.get(timeout=SilentBot.BUFFER) 
                    if buffer:
                        self._mic_device.write_frames(buffer)
                else :
                    time.sleep(SilentBot.BUFFER)
            except queue.Empty:
                continue
